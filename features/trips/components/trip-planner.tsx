...
import { ArrowUpRight, CalendarDays, ChevronDown, CircleDollarSign, Clock3, MapPin, Sparkles, Users } from "lucide-react";
import { useRouter } from "next/navigation";
import { Button } from "@/components/ui/button";
import { Progress } from "@/components/ui/progress";
import { Toaster } from "@/components/ui/sonner";
import { toast } from "sonner";
import { createTrip as createTripRequest, createTripSchedule, deleteTrip as deleteTripRequest, deleteTripSchedule, getTripDays, getTrips, updateTripMembers, updateTripSchedule } from "../api/trip-api";
import type { CreateTripInput, NewScheduleInput, ScheduleItem, ScheduleKind, Trip, UpdateScheduleInput } from "../model/trip";
...
export function TripPlanner({ tripId }: TripPlannerProps) {
  ...
  const currentTrip = useMemo(() => trips.find((trip) => trip.id === tripId) ?? null, [trips, tripId]);
  ...
  useEffect(() => {
    ...
    void getTripDays(tripId, controller.signal)
      ...
      .catch((error: unknown) => {
        ...
        setDays([]);
        setActiveDayId("");
        if (tripId !== "empty") toast.error("선택한 여행의 일정을 불러오지 못했어요.");
      });
    return () => controller.abort();
  }, [tripId]);

  const createNewTrip = useCallback(async (input: CreateTripInput) => {
    const created = await createTripRequest(input);
    setTrips((current) => [...current, created]);
    router.push(`/trips/${created.id}`);
  }, [router]);

  const removeTrip = useCallback(async (trip: Trip) => {
    try {
      await deleteTripRequest(trip.id);
      const remaining = trips.filter((candidate) => candidate.id !== trip.id);
      setTrips(remaining);
      if (trip.id === tripId) router.push(remaining[0] ? `/trips/${remaining[0].id}` : "/trips/empty");
  ...
  const saveMembers = useCallback(async (members: string[]) => {
  ...
  useEffect(() => {
    ...
    const registration = context.registerTool({
      ...
      async execute(input: unknown) {
        ...
        if (typeof value.dayId !== "string" || !days.some((day) => day.id === value.dayId)
  ...
  const dateRange = currentTrip ? formatTripDateRange(currentTrip) : "여행을 선택해 주세요";
  const tripLength = currentTrip ? Math.round((new Date(currentTrip.endDate).getTime() - new Date(currentTrip.startDate).getTime()) / 86400000) + 1 : 0;
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <header className="topbar">
        ...
        <nav aria-label="주요 메뉴" className="main-nav"><TripSwitcher trips={trips} currentTrip={currentTrip} onSelect={(trip) => router.push(`/trips/${trip.id}`)} onCreate={createNewTrip} onDelete={removeTrip} /><a className="active" href="#plan">여행 계획</a><a href="#budget">공동 경비</a><a href="#decisions">결정 보드</a></nav>
        <Button className="invite-button"><Users /> 초대하기</Button>
      </header>

      <div className="app-shell">
        <aside className="trip-rail" aria-label="여행 정보">
          <div><p className="eyebrow">선택한 여행</p><h2>{currentTrip?.title ?? "여행을 선택해 주세요"}</h2><div className="trip-meta"><CalendarDays /><span>{dateRange}<br />{currentTrip && <strong>{tripLength - 1}박 {tripLength}일</strong>}</span></div></div>
          {currentTrip && <div className="people-block"><div className="people-heading"><p className="eyebrow">함께 가는 사람</p><ManageMembersDialog key={currentTrip.id} trip={currentTrip} onSave={saveMembers} /></div><div className="avatars" aria-label={`참여자 ${currentTrip.members.length}명`}>{currentTrip.members.map((member, index) => <span className={`avatar a${index % 4 + 1}`} key={`${member}-${index}`}>{member.slice(0, 1)}</span>)}</div><p>{currentTrip.members.length}명이 계획 중</p></div>}
          ...
          <button className="switch-trip" onClick={() => document.querySelector<HTMLButtonElement>(".trip-switcher-trigger")?.click()}>다른 여행 보기 <ChevronDown /></button>
        </aside>

        <section className="plan-canvas" id="plan">
          {currentTrip && activeDay ? <>
            <div className="plan-heading">
              <div><p className="kicker"><Sparkles /> 모두의 선택을 한 흐름으로</p><h1>{currentTrip.destination} 일정</h1></div>
            ...
            <div className="day-tabs" style={{ gridTemplateColumns: `repeat(${days.length}, minmax(88px, 1fr))` }} role="tablist" aria-label="여행 날짜">
            ...
            <div className="day-summary"><div><span>{activeDay.date}</span><strong>{activeDay.items.length}개의 일정</strong></div><p><span className="save-indicator">Python API에 자동 저장</span> · 여행별로 독립된 일정입니다.</p></div>
            ...
            <EditScheduleDialog key={editingItem?.id ?? "closed"} item={editingItem} open={editingItem !== null} onOpenChange={(open) => { if (!open) setEditingItem(null); }} onSave={(input) => { if (editingItem) return updateSchedule(activeDayId, editingItem.id, input); }} />
          </> : <div className="no-trip"><MapPin /><h1>계획할 여행을 선택해 주세요</h1><p>상단의 여행 선택 메뉴에서 기존 여행을 고르거나 새 여행을 만들 수 있습니다.</p><Button onClick={() => document.querySelector<HTMLButtonElement>(".trip-switcher-trigger")?.click()}>여행 목록 열기</Button></div>}
        </section>

        <aside className="insight-rail">
          <div className="destination-card"><div className={`destination-image ${currentTrip?.id === "kyoto-autumn" ? "kyoto" : "generic"}`} role="img" aria-label={`${currentTrip?.destination ?? "여행지"} 이미지`} /><div className="destination-caption"><span>{currentTrip?.destination.toUpperCase() ?? "NEXT DESTINATION"}</span><strong>{currentTrip?.title ?? "우리의 다음 장면"}</strong></div></div>
          <section className="decision-card" id="decisions"><div className="section-icon"><CircleDollarSign /></div><p className="eyebrow">오늘 결정할 것</p><h2>{currentTrip?.id === "kyoto-autumn" ? <>료칸 조식,<br />추가할까요?</> : <>{currentTrip?.destination ?? "다음 여행"}의 첫 활동,<br />정해볼까요?</>}</h2><div className="vote-track"><span style={{ width: "75%" }} /></div><div className="vote-result"><strong>찬성 3</strong><span>대기 1</span></div><Button variant="outline" className="vote-button">투표 보러가기 <ArrowUpRight /></Button></section>
