...
- `src/features/trips/model/trip.ts`: 여행 일정의 도메인 타입
- `src/features/trips/components/trip-planner.tsx`: 날짜 선택과 화면 상태 관리
- `src/features/trips/components/add-schedule-dialog.tsx`: 입력 폼과 검증
- `src/features/trips/components/schedule-timeline.tsx`: 일정 종류별 렌더링
- `src/app/trips/[tripId]/page.tsx`: 여행 ID를 받는 동적 라우트
...
1. `src/features/trips/model/trip.ts`에서 `ScheduleItem`의 세 종류를 비교합니다.
2. `src/features/trips/api/trip-api.ts`에서 JSON을 Zod로 검증하는 과정을 확인합니다.
...
6. `backend/app/domain/schemas.py`와 TypeScript 타입을 비교하고 두 언어가 API 계약을 표현하는 방식을 확인합니다.
7. `backend/app/api/routes`에서 URL이 기능별로 나뉘는 방식을 확인합니다.
8. `backend/app/infrastructure/database.py`에서 SQL의 생성·조회·수정·삭제 흐름을 따라갑니다.
...
5. Sites 관리형 D1/R2로 배포용 저장소를 구성합니다.
6. GitHub Actions CI가 통과한 버전을 Sites에 배포합니다.
